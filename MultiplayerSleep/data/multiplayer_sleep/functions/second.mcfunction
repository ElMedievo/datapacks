# Warns players every night based on time query and trigger toggled
#
# Called by: #main:second

execute store result score #ms_time ms_time run time query daytime
execute if score #ms_warnToggle ms_warnToggle matches -1 if score #ms_time ms_time matches 12500..24000 if entity @a[nbt={Sleeping:1b}] run tellraw @a[tag=!ms_warn] ["",{"text":"Un jugador no quiere dormir.", "color": "yellow"}]
